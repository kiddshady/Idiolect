...
contextBridge.exposeInMainWorld('onyx', {
  ...
  /** Actualizaciones desde los releases de GitHub (ver src/actualizador.cjs). */
  update: {
  ...
  /** Idiolect: regenerar glosario.md y enterarse de lo que escribe otro (Claude, a mano). */
  glosario: {
    exportar: () => call('glosario:exportar'),
    onCambio: (cb) => {
      const handler = () => cb();
      ipcRenderer.on('entradas:cambio', handler);
      return () => ipcRenderer.off('entradas:cambio', handler);
    },
  },
});
