...
/* Idiolect: la carpeta es FIJA, la corra quien la corra (el repo, la app
   instalada o `tools/anotar.cjs` desde el shell de Claude). Tiene que ser la
   misma para los tres porque el CLAUDE.md global importa el glosario.md que
   vive acá. Queda fuera de AppData a propósito: el sandbox de Claude
   virtualiza el perfil de usuario y las dos partes verían carpetas distintas. */
const ROOT = path.resolve(process.env.IDIOLECT_DATA || 'S:/tools/Idiolect/data');
...
module.exports = {
  ...
  ensureDirs, readJSON, writeJSON, renameWithRetry, assertId,
