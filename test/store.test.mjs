...
await store.writeJSON(store.SETTINGS_FILE, { ...base, otra: 'queda' });
const parche = await store.saveSettings({ ultimaVista: 'revisar' });
ok('guardar un parche no pisa el resto', parche.ultimaVista === 'revisar' && parche.otra === 'queda');
// Una clave nueva del código tiene que aparecer en un archivo viejo.
await store.writeJSON(store.SETTINGS_FILE, { schema: store.SCHEMA, otra: 'queda' });
const completado = await store.loadSettings();
ok('las claves nuevas se completan solas', 'ultimaVista' in completado && completado.otra === 'queda');
